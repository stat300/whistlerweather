...
\end{center}
\vspace{-5mm}
\caption{Snowfall is trending downards at a rate of \Sexpr{round(abs(coef(snowmodel)[2])*365,2)} cm per year}
\label{fig:snowtrend}
\end{figure}
...
summary_table[,8] <- paste(round(summary_table[,8],2), "c")
